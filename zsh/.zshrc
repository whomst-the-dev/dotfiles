...
# Sourced in interactive shells (after zshenv and zprofile)
# Contains: functions, options, key bindings

# Enable colors and change prompt:
autoload -U colors && colors	# Load colors
NEWLINE=$'\n'
PS1="%B%{$fg[red]%}┌─[%{$fg[yellow]%}%n%{$fg[green]%}@%{$fg[blue]%}%M %{$fg[magenta]%}%~%{$fg[red]%}]${NEWLINE}│${NEWLINE}└─%{$reset_color%}$%b "
...
# Function for test shell load time
timezsh(){
    shell=${1-$SHELL}
